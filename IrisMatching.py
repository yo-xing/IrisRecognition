...
def IrisMatching(X_train, y_train, X_test, y_test, num, keep_dim, method = "LDA"):
    ...
    #lists of correctness (1 or 0) for each test image's indentification
    correct_L1=[] 
    correct_L2=[]
    correct_cosine=[]
    correct_cosine_ROC=[]
    ...
    #checking the ROC thresholds for cosine distance and recording them for each test image
    for x in range(len(thresholds)):
        correct_ROC=[]
        for y in range(len(min_cosine)):
            correct_ROC.append(1*(min_cosine[y]<=thresholds[x]))
        correct_cosine_ROC.append(correct_ROC)
        
    
    #checking correctness for each test image on each similarity measure
    for k in range(0,len(list_L1)):
        ...
        correct_L1.append(1*(list_L1[k] in range(match,match+3)))
        correct_L2.append(1*(list_L2[k] in range(match,match+3)))
        correct_cosine.append(1*(list_cosine[k] in range(match,match+3)))
